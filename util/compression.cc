...
Status LZ4Compressor::Compress(const CompressionInfo& info, const Slice& input,
                               std::string* output) {
  bool success = LZ4_Compress(info, input.data(), input.size(), output);
  if (UNLIKELY(!success)) {
    return Status::Corruption();
  }
  return Status::OK();
...
class ZSTDContext {
 public:
  ZSTDContext() {
    ...
    cctx_ = ZSTD_createCCtx_advanced(port::GetJeZstdAllocationOverrides());
  ...
  ZSTD_DCtx* GetUncompressionContext() const {
    return uncomp_cached_data_.Get();
  }
  ZSTD_CCtx* GetCompressionContext() const { return cctx_; }
...
static thread_local ZSTDContext zstd_context;
...
Status ZSTDCompressor::Compress(const CompressionInfo& info, const Slice& input,
  ...
  size_t outlen = 0;
  int level;
  if (compression_opts_.level == CompressionOptions::kDefaultCompressionLevel) {
    // 3 is the value of ZSTD_CLEVEL_DEFAULT (not exposed publicly), see
    // https://github.com/facebook/zstd/issues/1148
    level = 3;
  } else {
    level = compression_opts_.level;
  }
#if ZSTD_VERSION_NUMBER >= 500  // v0.5.0+
  ZSTD_CCtx* context = zstd_context.GetCompressionContext();
...
Status ZSTDCompressor::Uncompress(const UncompressionInfo& info,
  ...
  if (!compression::GetDecompressedSizeInfo(&input, &input_length,
                                            &output_len)) {
    return Status::Corruption("ZSTD: Cannog determine compressed size");
  }

  auto output = Allocate(output_len, info.GetMemoryAllocator());
  size_t actual_output_length = 0;
#if ZSTD_VERSION_NUMBER >= 500  // v0.5.0+
  ZSTD_DCtx* context = zstd_context.GetUncompressionContext();
  assert(context != nullptr);
