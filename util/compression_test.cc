...
TEST(Compression, TestBuiltinCompressors) {
  ...
  config_options.ignore_unsupported_options = false;
  config_options.ignore_unknown_options = false;
  CompressionOptions compression_opts(1, 2, 3, 4, 5, 6, false, 7, false);
...
TEST(Compression, ColumnFamilyOptionsFromStringWithAddedCompressor) {
  ConfigOptions config_options;
  auto library = config_options.registry->AddLibrary(
      "ColumnFamilyOptionsFromStringWithAddedCompressor");
  std::vector<std::shared_ptr<Compressor>> compressors;
  ColumnFamilyOptions options, new_options;
  config_options.ignore_unsupported_options = false;
...
TEST(Compression, DBwithZlibAndCompressorOptions) {
  ...
  compression_opts.window_bits = -13;
  options.compressor =
      BuiltinCompressor::GetCompressor(kZlibCompression, compression_opts);
#endif
  ASSERT_NE(options.compressor, nullptr);
  ...
  s = LoadLatestOptions(config_options, db->GetName(), &db_options, &cf_descs);
  ASSERT_OK(s);
  // CompressionOptions are still default
...
TEST(Compression, DBWithDictionaryCompression) {
  ...
  ConfigOptions config_options;
  auto library =
      config_options.registry->AddLibrary("DBWithDictionaryCompression");
  AddCompressorFactory<DummyDictionaryCompressor>(library);
  Status s = Compressor::CreateFromString(
      config_options,
