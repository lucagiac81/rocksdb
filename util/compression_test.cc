...
TEST(Compression, ColumnFamilyOptionsFromStringWithCompression) {
  ...
  ASSERT_NE(new_options.compressor, nullptr);
  // TODO Zlib fails to override default Snappy if using the "compression"
  // option (it correctly overrides if using the "compressor" option). Listed as
  // limitation pending fix.
