...
Slice CompressBlock(Compressor* compressor, const Slice& raw,
  ...
  if (do_sample && info.SampleForCompression() &&
    ...
    if (sampled_output_fast && (LZ4_Supported() || Snappy_Supported())) {
      ...
      info_tmp.CompressData(sampler.get(), raw, sampled_output_fast);
    }
    ...
    if (sampled_output_slow && (ZSTD_Supported() || Zlib_Supported())) {
      ...
      info_tmp.CompressData(sampler.get(), raw, sampled_output_slow);
    }
  }
  ...
  *type = compressor->GetCompressionType();
  ...
  if (!info.CompressData(compressor, raw, compressed_output)) {
    *type = kNoCompression;
    return raw;
  }
...
void BlockBasedTableBuilder::CompressAndVerifyBlock(
  ...
  *type = r->compressor->GetCompressionType();
  ...
  if (is_status_ok && raw_block_contents.size() < kCompressionSizeLimit) {
    ...
    *block_contents =
        CompressBlock(r->compressor.get(), raw_block_contents, compression_info,
                      type, is_data_block /* do_sample */, compressed_output,
  ...
  } else if (*type != r->compressor->GetCompressionType()) {
    RecordTick(r->ioptions.stats, NUMBER_BLOCK_NOT_COMPRESSED);
  }
}
  ...
  if (block_cache != nullptr) {
    ...
    std::unique_ptr<TBlocklike> block_holder(
        BlocklikeTraits<TBlocklike>::Create(
            ...
            rep_->ioptions.statistics.get(), nullptr,
